...
### Q: TAURI_SIGNING_PRIVATE_KEY 或 updater 配置错误是什么？

A: 这是 Tauri 应用更新器签名相关的错误。工作流已自动处理此问题：

**Token 说明**:
- **GITHUB_TOKEN**: GitHub 自动提供，用于仓库操作（无需配置）✅
- **TAURI_SIGNING_PRIVATE_KEY**: 工作流自动生成临时密钥对 ✅

**自动处理流程**:
1. 在构建前自动生成临时的签名密钥对
2. 使用临时公钥替换配置文件中的 `pubkey`
3. 在构建时通过环境变量提供私钥
4. 构建完成后自动清理临时文件

这样既能通过 Tauri 的签名验证，又不需要手动配置密钥。生成的应用包可以正常使用，只是不支持自动更新功能（对于自定义打包场景通常不需要）。
